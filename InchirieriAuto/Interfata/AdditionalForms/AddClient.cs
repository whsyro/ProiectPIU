...
namespace Interfata.AdditionalForms
{
    public partial class AddClient : Form
    {
        ...
        {
            txtBoxNume.Text = textBoxCNP.Text = textBoxPrenume.Text = textBoxUsername.Text = textBoxPassword.Text = textBoxAn.Text = textBoxCuloare.Text = textBoxMarca.Text = textBoxVin.Text = string.Empty;
        }
        ...
        {
            if (txtBoxNume.Text != string.Empty && 
                textBoxCNP.Text != string.Empty && 
                textBoxPrenume.Text != string.Empty && 
                textBoxUsername.Text != string.Empty && 
                textBoxPassword.Text != string.Empty && 
                Admin.VerifExistFile("clienti.csv") &&
                textBoxMarca.Text != string.Empty &&
                textBoxAn.Text != string.Empty &&
                textBoxCuloare.Text != string.Empty &&
                textBoxVin.Text != string.Empty&&
                Admin.VerifExistFile("auto.csv")
                )                
            {
                int nradministratori = 0;
                Admin admin = new Admin();
                Persoana[] admini = admin.GetPersoana(out nradministratori, "clienti.csv");
                Admin.ScriereCsvPersoana("clienti.csv", admini[nradministratori - 1].GetIDPersoana() + 1, txtBoxNume.Text, textBoxPrenume.Text, textBoxCNP.Text, textBoxUsername.Text, textBoxPassword.Text);

                int nrAuto = 0;
                Admin administrator= new Admin();
                ClassAuto[] auto = administrator.GetAuto(out nrAuto, "auto.csv");
                Admin.ScriereCsvAuto("auto.csv", auto[nrAuto - 1].GetIDAuto() + 1, textBoxMarca.Text, textBoxCuloare.Text, textBoxAn.Text, textBoxVin.Text, "DA");
